...
const QuestionDetail = () => {
  ...
  const submitHanlder = async (values) => {
    try {
      await axios.post("http://localhost:3001/question", { ...values });
  ...
  if (error === "b")
    return <p style={{ position: "absolute", left: "48%", top: "53%" }}></p>;
  return (
    <>
   ...
   <div className={styles.edtitorContainer}>
   <ReactQuill className={styles.editor} modules={modules} />
   </div>
      </div>
